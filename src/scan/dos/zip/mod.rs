...
mod tests {
    ...
    fn entries_parses_a_well_formed_archive() {
        ...
        let entries = entries(&data).expect("expected Some entries");
    ...
    fn detects_vba_project_marker() {
        ...
        assert!(zip_has_active_content(&data).is_some());
    }

    #[test]
    fn no_marker_means_no_active_content() {
        let data = build_zip(&[(b"word/document.xml", 10, 10)]);
        assert_eq!(zip_has_active_content(&data), None);
    }
    ...
    fn malformed_archive_is_treated_as_risky() {
        let data = b"garbage";
        assert_eq!(zip_has_dos_risk(data), Some(0));
    }
